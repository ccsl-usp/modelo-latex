...
% metadados XMP). Estas macros "apagam" esses comandos; basta usá-las em
% um grupo para que a mudança não seja permanente.
...
% Agora inserimos de fato os metadados XMP no arquivo PDF
\makeatletter

% Se o documento é uma tese/dissertação no formato do IME, a definição dos
% metadados é mais complexa e já foi preparada em thesis-formatting.tex;
% Se não é tese/dissertação, colocamos apenas os dados essenciais.
\def\@IMEsimpleXMP{
  % Remove quebras de linha etc.
