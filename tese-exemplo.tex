...
% Define a língua do documento (e línguas adicionais) para cuidar dos nomes
% das seções ("chapter" X "capítulo" etc.), hifelização e outras convenções
% tipográficas. A última língua da lista é a língua padrão do documento. babel
% deve ser um dos primeiros pacotes carregados.
...
% LaTeX às vezes coloca notas de rodapá logo após o final do texto da
...
% Os nomes "left/right/both" aqui não fazem muito sentido, melhor
% pensar neles como palavras "mágicas". Para fazer mudanças não
% triviais aqui é necessário ler a documentação.
%
...
%
% "RO" significa "Right side of Odd pages"; "LE" significa "Left side
% of Even pages" etc.

\fancypagestyle{mainmatter}{ %
  % Nome do capítulo no cabeçalho das páginas pares
  \renewcommand{\chaptermark}[1]{\markboth{##1}{} }

  % Número e nome da seção no cabeçalho das páginas ímpares
  \renewcommand{\sectionmark}[1]{\markright{\thesection. ##1} }

  % Limpa todas as configurações de cabeçalho e rodapé
  \fancyhf{}
  ...
  \fancyhead[RO]{\hfill\footnotesize{\thepage}}
  \fancyhead[LE]{\footnotesize{\thepage}\hfill}
  \fancyhead[LO]{\footnotesize{\MakeUppercase{\rightmark}}\hfill}
  \fancyhead[RE]{\hfill\footnotesize{\MakeUppercase{\leftmark}}}

  % Em negrito
  %\fancyhead[RO]{\hfill\footnotesize{\textbf{\thepage}}}
  %\fancyhead[LE]{\footnotesize{\textbf{\thepage}}\hfill}
  %\fancyhead[LO]{\footnotesize{\textbf{\rightmark}}\hfill}
  %\fancyhead[RE]{\hfill\footnotesize{\textbf{\leftmark}}}
}
...
% Permite imprimir páginas específicas em modo paisagem
\usepackage{pdflscape}

% Permite girar elementos na página
\usepackage{rotating}
...
% faz a referência apontar para o início do float (é possível customizar
% também).
...
% titlesec, que mencionamos mais abaixo. Se você pretende criar bibliografias
...
% resumo, além do comando "\epigrafe". Se você pretende criar essas páginas
% manualmente, não precisa carregar este pacote nem definir os dados abaixo
% (mas veja que \title e \author são usados para armazenar metadados no PDF
% gerado, então devem sempre ser definidos).
\usepackage{imeusp}
...
% Entradas customizadas para o índice remissivo (opcionais, pois o índice é
% opcional). É comum colocar esses itens no final do documento, junto com o
% comando \printindex, mas em alguns casos isso torna necessário executar
% texindy (ou makeindex) mais de uma vez, então colocar isto aqui é melhor.
...
% Após a capa e as páginas de rosto, começamos a numerar as páginas
\pagenumbering{roman}
