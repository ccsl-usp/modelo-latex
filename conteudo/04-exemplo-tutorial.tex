...
overleaf (\url{www.overleaf.com}) é o mais usado; \url{www.tex4all.com}
é uma alternativa similar.
...
\section{Limitações do \LaTeX{} e alternativas}
...
\begin{itemize}
    ...
    \item A linguagem é muito prolixa: é tedioso escrever coisas como
    ``\ltxcmd{begin\linebreak[0]{}\{itemize\}}'' etc. Linguagens como
    asciidoc (\url{asciidoctor.org}), markdown (\url{commonmark.org}),
    bookdown (\url{bookdown.org}) e reStructuredText (\url{sphinx-doc.org})
    operam de maneira similar a \LaTeX{}, mas sua sintaxe é bem mais
    enxuta. Elas funcionam muito bem para a geração de páginas web,
    mas \LaTeX{} oferece mais recursos e geralmente produz resultados
    impressos melhores, além de possuir modelos pré-prontos para um
    grande número de publicações científicas, especialmente nas áreas
    de matemática, ciência da computação, física e afins. typst
    (\url{typst.app}) busca ser um ``\LaTeX{} melhorado`` e, embora
    ainda seja relativamente jovem e pouco conhecido, já é bastante
    funcional.
    ...
    Seções~\ref{sec:quebras} e \ref{sec:exemplos-graficos}).
    ...
    outras publicações é baseada nas classes padrão. Os desenvolvedores atuais
    do \LaTeX{} vêm trabalhando para minimizar essas restrições, mas o processo
    é lento e limitado pela necessidade de manter a compatibilidade com versões
    mais antigas da linguagem. Con\TeX{}t é um ``irmão mais novo'' de \LaTeX{}
    com diversas vantagens, mas com sintaxe diferente e que ainda não é tão
    popular.
\end{itemize}
