...
\begin{multicols}{2}
  ...
  \begin{itemize}
    ...
    \item @inProceedings (ou @conference): \cite{dbevolution}

    \item @inCollection (capítulo de livro ou coletânea):
          \cite{gridpat}

    \item @phdThesis: \cite{curie}

    \item @mastersThesis: \cite{shannon}

    \item @techreport: \cite{yoon23}

    \item @manual: \cite{biblatex}

    \item @online (textos \emph{online} com autoria, como postagens ou
          comentários em blogs, mensagens de email etc.): \cite{gnu83}

    \item @online (para referência não a textos específicos,
          mas a sítios web como um todo): \cite{FSF}
  \end{itemize}
\end{multicols}
...
Para citar material \emph{online}, há quatro casos:
...
\begin{itemize}
  ...
  ou o \textsf{doi}, que direciona para o endereço correto.
  \looseness=-1
  ...
  mas sim um sítio (como uma empresa ou produto), pode ser mais adequado
  colocar a referência apenas como nota de rodapé e não na lista de
  referências. Outra opção é criar uma segunda lista de referências
  especificamente para recursos \emph{online} desse tipo
  (biblatex\index{biblatex} permite criar múltiplas bibliografias).

  \item Embora seja possível tratar artefatos de software da mesma
  maneira, é melhor seguir as recomendações do projeto \emph{Software
  Heritage}~\citep{soft-with-style}, que têm suporte com biblatex
  neste modelo (por exemplo, podemos citar uma versão específica de
  um software com~\cite{mlh-archiver}).
\end{itemize}
...
equação tem zero, uma ou duas raízes reais~distintas''.\looseness=-1
...
\begin{figure}
  ...
  \begin{subfigure}{0.4\textwidth}
    \centering
    \includegraphics[width=.55\textwidth]{exemplo-grafo}
...
\begin{verbatim}
  \begin{figure}
  \adjustbox{center}{\includegraphics[width=1.2\textwidth]{img}}
  \caption{...}
  \end{figure}
\end{verbatim}
...
\begin{figure}
  ...
  % Package pgfgantt; veja o arquivo imelooks.sty, em que vários
...
\begin{itemize}
  ...
  \item Se cabe em uma página mas é muito larga e precisa ser impressa em
        modo paisagem, use \textsf{sidewaystable}\footnote{Existe também
        \textsf{sidewaysfigure}.} (que também é um \textit{float});
  \item Se não cabe em uma página por ser muito longa, use \textsf{longtable}
        (que \emph{não} é um float);
