...
Para compilar o documento, basta executar o comando \textsf{make} (ou
\textsf{latexmk} se ele estiver instalado). \LaTeX{}
gera diversos arquivos auxiliares durante a compilação que, em algumas raras
situações, podem ficar inconsistentes (causando erros de compilação ou
erros no PDF gerado, como referências faltando ou numeração de páginas
incorreta no sumário). Nesse caso, é só usar o comando \textsf{make clean},
que apaga todos os arquivos auxiliares gerados, e em seguida rodar
\textsf{make} novamente.
